...
if (! nv_function_exists('nv_block_news_groups_center')) {
    function nv_block_config_news_groups_center($module, $data_block, $lang_block)
    {
    ...
    function nv_block_config_news_groups_center_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_block_news_groups_center($block_config)
    {
        ...
        if (! empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_groups_center.tpl')) {
                $block_theme = $global_config['module_theme'];
            } else {
                $block_theme = 'default';
            }
            $xtpl = new XTemplate('block_groups_center.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news');
            ...
            foreach ($list as $key=>$l) {
                ...
                $l['publtimed'] = nv_date('d', $l['publtime']);
                $l['publtimem'] = nv_date('m', $l['publtime']);
                $l['publtimey'] = nv_date('Y', $l['publtime']);
                $l['cattitle'] = $module_array_cat[$l['catid']]['title'];
                $l['catlink'] = $module_array_cat[$l['catid']]['link'];
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_block_news_groups_center($block_config);
    }
}
